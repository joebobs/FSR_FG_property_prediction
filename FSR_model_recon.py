...
def FSR_CV(config):
    ...
    #data_df = data_df.sample(70000, random_state=42)
    ...
    for fold_ind in range(config['num_folds']):
        ...
        # create dataloaders
        partition_sup = {'train': train_indices}
        ...
        for tr_epo in range(config['epochs']):
            for i, batch in enumerate(training_generator_sup):
                ...
                if config['train_component'] == 'mainAE':
                    loss_r = config['reconstruction_coefficient'] * F.binary_cross_entropy(recon, v_D)
                elif config['train_component'] == 'categoricalAE':
                    assert torch.is_tensor(cat_recon)
                    loss_r = config['reconstruction_coefficient'] * F.binary_cross_entropy(cat_recon, features_batch[0].float())
                elif config['train_component'] == 'numericalAE':
                    assert num_recon
                    loss_r = config['reconstruction_coefficient'] * F.binary_cross_entropy(num_recon, features_batch[1].float()) # Create another rec. coeff if this is to be used
                else:
                    raise ValueError
                ...
                if(i % 20 == 0):
                    print('Training at Epoch ' + str(tr_epo) + ' iteration ' + str(i) + 'recon loss is ' + '%.8f' %(loss_r.cpu().detach().numpy()) + ', avg: %.8f' %(np.mean(loss_r_history[-20:])), 'LR: %f' %(opt.param_groups[0]['lr']))
                #if i > 12000:
                #    break
        ...
        print(f"\nBest Score: {deciding_metric}  - {best_score} on epoch {best_epoch}\n")
        '''with torch.set_grad_enabled(False):
    ...
    return 0, 0, 0, 0, 0 #mean_test_score, std_test_score, mean_val_score, std_val_score, all_test_scores_dict
